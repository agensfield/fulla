...
| Store `.fulla-lock-release-v1-HOST-PID-TOKEN` | Original lock metadata only, never private keys or entry data | [Atomic detach](lock-release-recovery.md) retains cleanup ownership through every removal. Explicit dead-owner recovery preserves a newer writer; unknown contents refuse. |
| `.fulla/basic-v1/transactions/ID` and `backups/ID` | Encrypted entry data and encrypted Git history only | [Fixed basic-write ownership](basic-write-protocol.md) binds stages to the shared lock and routes explicit cleanup independently of newer feature metadata. No private identities are staged here. |
