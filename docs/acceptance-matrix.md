...
| J4 | Two-host operation | [Current/previous sessions](../internal/remote/remote_test.go), [one-sided commit/retry](../internal/remote/partial_test.go), real OpenSSH loopback in J2; historical Mac/devbox receipt in the development ledger. | [Physical-host partial retry](crosshost-acceptance.md) now passes macOS/arm64 to devbox Linux/x86_64 on Git/no-Git stores at two lost-reply boundaries, with partial receipts, exact bytes and unchanged committed ciphertext. Fixture pins, current protocol and synthetic stores; CLI enrollment, previous-protocol cases, physical power loss and live cutover remain separate evidence. |
...
| J6 | Identity lifecycle | [Continuity/destruction](../internal/store/rotation_test.go), [killed rotation](../internal/store/rotation_crash_test.go), explicit historical recovery and sealed chains. | [Private-copy audit](private-copy-audit.md) inventories owned disk paths and found a missing atomic-key-file guard. Doctor and destructive rotation/recovery now detect reserved atomic siblings beside identities and retired keys; reconstructed-state and previous-source controls verify refusal without mutation. Unbound atomic/init/restore cleanup and physical durability remain open, including interruption before ownership is published. |
...
| F9 | Disjoint/equal/different inventories and one-sided commit | Remote session and partial-retry fixtures, plus J2 OpenSSH integration. | Bounded protocol evidence; physical-host lost-reply acceptance is recorded in J4. |
...
| I6 | Explicit secret input/output channels only | stdin/fd/TTY/editor/clipboard/run surfaces; raw and base64 show tests. | [Export input preflight](machine-contract-matrix.md#export-eligibility-before-recovery-input) now proves impossible publication/malformed manifest refusals leave the selected passphrase descriptor untouched. Complete per-command flag applicability and channel matrix remain J3 work. |
| I7 | No argv/ambient secret-value input or diagnostic disclosure | Parser/input tests, fixed failure messages, plugin debug guard, diagnostic redaction and panic handling. | Complete error-path and subprocess environment/output audit, including plugin waits and Git execution. [Internal Git output](git-routing.md) now has bounded stdout, cancellation/reaping on excess and no partial result; active-operation deadlines and arbitrary descendant behavior remain separate. |
...
| Preview `v0.1.0` | CI covers race/vet, TTY, clipboard, real pa, and reproducible packages. | Complete required journeys, compatibility/migration gates, known limitations, [Tag-gated workflow](distribution.md) is implemented; actual hosted publication/attestation/asset verification and tagged go-install proof remain open. [Public commit installation](distribution.md) passed hosted Linux/amd64 and macOS/arm64 at reviewed 8b5affb in run 34007934062, with downloaded receipts confirming fresh caches, module provenance and installed-binary exact-byte/deep-health checks. It is not tagged-release or other-native-architecture acceptance. |
| Agensfield Homebrew tap | [Formula generator](distribution.md) validates package hashes and expected binary provenance, emits four-platform URLs/checksums, completions and a no-Git read/write test. Synthetic refusal cases and Ruby syntax pass. [Hosted fixture installation](distribution.md#hosted-homebrew-installation-drill) passed actual macOS/arm64 install, generated completions and brew test in run 34009919496, with downloaded receipt and cleanup. | Generate from accepted release artifacts and publish only Fulla's formula in the real Agensfield tap; tagged/public-URL and other-native-platform installation remain open. The fixture used an ephemeral runner tap only. |
