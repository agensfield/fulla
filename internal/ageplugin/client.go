...
import (
	...
	"filippo.io/age"
	upstream "filippo.io/age/plugin"
	"github.com/agensfield/fulla/internal/ageplugin/format"
)
...
func NewRecipient(s string, ui *ClientUI) (*Recipient, error) {
	name, _, err := upstream.ParseRecipient(s)
	if err != nil {
		return nil, err
	}
	return &Recipient{
...
func NewIdentity(s string, ui *ClientUI) (*Identity, error) {
	name, _, err := upstream.ParseIdentity(s)
	if err != nil {
		return nil, err
	}
	return &Identity{
...
func NewIdentityWithoutData(name string, ui *ClientUI) (*Identity, error) {
	s := upstream.EncodeIdentity(name, nil)
	if s == "" {
...
func (cc *clientConnection) Close() error {
	...
	cc.close()
	_ = cc.cmd.Process.Signal(os.Interrupt)
	done := make(chan error, 1)
	go func() { done <- cc.cmd.Wait() }()
	...
	case err := <-done:
		return err
	case <-timer.C:
		// Only bound cleanup after the protocol has finished or failed. This
		// does not impose a deadline on hardware touch or PIN interaction.
		_ = cc.cmd.Process.Kill()
		return <-done // Reap the owned child before returning to the caller.
	}
}
