...
func TestDoctorCreationRequiresBoundStage(t *testing.T) {
	for _, kind := range []string{"init", "restore"} {
		t.Run(kind, func(t *testing.T) {
			for _, binding := range []string{"valid", "absent", "wrong-target", "conflicting"} {
				t.Run(binding, func(t *testing.T) {
					...
					stage := filepath.Join(home, ".fulla-"+kind+"-"+id)
					...
					info := fmt.Sprintf("pid=%d host=%s operation=%s", child.ProcessState.Pid(), host, kind)
					if binding != "absent" {
						boundTarget := target
						if binding == "wrong-target" {
							boundTarget = filepath.Join(home, "elsewhere", "target")
						}
						info += " " + kind + "_id=" + id + " " + kind + "_target=" + base64.RawURLEncoding.EncodeToString([]byte(boundTarget))
					...
					if binding == "valid" {
						...
						if inspected != 1 || owner == nil || owner.Token != token || (kind == "init" && (owner.InitID != id || owner.InitTarget != target)) || (kind == "restore" && (owner.RestoreID != id || owner.RestoreTarget != target)) {
					...
					if binding == "valid" {
						if exit != 0 || !result.OK || result.Data[kind+"_applied"] != false {
