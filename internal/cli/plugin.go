...
// Plugin interaction uses the controlling terminal, never command input/output.
// Machine mode and remote protocol handlers cannot acquire an interactive UI.
func commandPluginUI(p invocation) *plugin.ClientUI {
	if p.has("json") || p.has("non-interactive") || strings.HasPrefix(p.Command, "remote ") {
		return nil
	}
	read := func(prompt string) ([]byte, error) {
		value, err := withTerminal("age plugin input requires a controlling terminal", func(ctx context.Context, tty *os.File) ([]byte, error) {
