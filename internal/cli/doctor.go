...
func (a *App) doctor(p invocation, c *config.Resolved, s *store.Store) (any, error) {
	if len(p.Args) != 0 {
	...
	if p.has("report") {
		...
		if err := s.CheckArtifactPath(p.value("report")); err != nil {
	...
	if p.has("report") {
		...
		if writeErr := s.PublishArtifact(p.value("report"), append(data, '\n')); writeErr != nil {
